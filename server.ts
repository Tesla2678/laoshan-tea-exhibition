  ...
  try {
    ...
    if (image_list && Array.isArray(image_list)) {
      for (const img of image_list) {
        ...
        imageParts.push({ inlineData: { data: base64Data, mimeType: 'image/png' } });
      }
    }

    if (maskImage) {
      imageParts.push({ inlineData: { data: maskImage.replace(/^data:image\/\w+;base64,/, ''), mimeType: 'image/png' } });
    }

    let textPrompt = prompt;
    const model = siteConfig.api.google.model.consultant; // gemini-3-pro-image-preview
