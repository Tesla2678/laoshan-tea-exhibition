...
export const siteConfig = {
  ...
  defaultConsultantPrompt: `作为专业展陈设计顾问，请参考我提供的参考图。
【结构参考图】（共0张，第1张为展位空间结构底稿）：严格保持第1张图中的所有空间结构、墙面隔断、出入口和视线通廊【完全不变】。
【风格参考图】（共0张）：请将这些图中的茶具、屏风、盆景等中式元素巧妙地融入到结构底稿空间中。
...
export function buildConsultantPrompt(structureCount: number, referenceCount: number): string {
  return `作为专业展陈设计顾问，请参考我提供的参考图。
【结构参考图】（共${structureCount}张，第1张为展位空间结构底稿）：严格保持第1张图中的所有空间结构、墙面隔断、出入口和视线通廊【完全不变】。
【风格参考图】（共${referenceCount}张）：请将这些图中的茶具、屏风、盆景等中式元素巧妙地融入到结构底稿空间中。
风格要求：自然质朴，山海气息，高级专业展陈效果图，灯光层次丰富，克制陈设（3-6件核心小品），打造崂山茶文化沉浸式质感。务必在不改变原建筑结构的前提下，呈现出照片级的高清真实光影效果！`;
}
